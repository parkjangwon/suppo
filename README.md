...
> 💡 **데모 데이터 안내**  
> 최초 실행 시 기능을 바로 확인할 수 있도록 샘플 티켓, 상담원, 고객, 지식베이스 등 **시드(seed) 데이터가 자동으로 삽입**됩니다.  
> 실제 운영에 앞서 데이터를 초기화하려면 아래 방법을 사용하세요.
>
> **관리자 콘솔에서 초기화하기 (권장)**
>
> 1. 관리자 패널(`http://localhost:3001/admin`) 로그인
> 2. 좌측 메뉴 → **시스템**
> 3. **시스템 초기화** 섹션에서 **모두 선택** 체크
> 4. **초기화** 버튼 클릭 → 확인 입력 후 실행
>
> 초기화 후에는 관리자 계정만 남고 나머지 데이터는 모두 삭제됩니다.
