...
export function WebhookEndpointManager() {
  ...
  const t = (key: string, ko: string, en?: string) =>
    (copy[key] ?? (locale === "en" ? (en ?? ko) : ko));
  const WEBHOOK_EVENTS = [
    { id: "ticket.created", label: t("webhookEventTicketCreated", "티켓 생성", "Ticket created") },
    { id: "ticket.updated", label: t("webhookEventTicketUpdated", "티켓 수정", "Ticket updated") },
    { id: "ticket.commented", label: t("webhookEventTicketCommented", "댓글 등록", "Ticket commented") },
  ];
  ...
  async function sendTestWebhook(webhookId: string) {
    ...
    try {
      ...
      toast.success(copy.webhookTestSuccess ?? "테스트 webhook을 발송했습니다.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : (copy.webhookTestFailed ?? "테스트 webhook 발송에 실패했습니다."));
    } finally {
      setTestingId(null);
    }
  }
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle>{copy.webhookTitle ?? "Outbound Webhook"}</CardTitle>
          <p className="text-sm text-muted-foreground">{copy.webhookDescription ?? "티켓 이벤트를 외부 URL로 POST 전송합니다."}</p>
        </div>
        <Button onClick={() => setIsDialogOpen(true)}>{copy.webhookAdd ?? "Webhook 추가"}</Button>
      </CardHeader>
      <CardContent className="space-y-4">
        {webhooks.length === 0 ? (
          <p className="text-sm text-muted-foreground">{copy.webhookNone ?? "등록된 Webhook이 없습니다."}</p>
        ) : (
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {testingId === webhook.id ? (copy.commonSending ?? "전송 중...") : (copy.commonSendTest ?? "테스트 발송")}
                    </Button>
                    <Switch
                ...
                <div className="rounded-md border bg-muted/10 p-3">
                  <div className="mb-2 text-sm font-medium">{copy.webhookHistoryTitle ?? "최근 호출 이력"}</div>
                  <div className="space-y-2">
                    {(deliveries[webhook.id] ?? []).length === 0 ? (
                      <div className="text-xs text-muted-foreground">
                        {copy.webhookHistoryEmpty ?? "아직 호출 이력이 없습니다. 먼저 테스트 발송을 눌러 연결 상태를 확인하세요."}
                      </div>
                    ) : (
                      deliveries[webhook.id].map((delivery) => (
                        <div key={delivery.id} className="rounded border bg-background p-2 text-xs">
                          ...
                          <div className="mt-1 text-muted-foreground">
                            {copy.webhookResponseCode ?? "응답 코드"}: {delivery.responseStatusCode ?? (copy.commonNone ?? "없음")}
                          </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{copy.webhookAddTitle ?? "Webhook 추가"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
