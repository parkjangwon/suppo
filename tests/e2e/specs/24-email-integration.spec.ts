...
const TEST_NOTIFICATION_EMAIL = "qa-email-e2e@suppo-test.io";
...
async function seedEmailSettings(overrides: Record<string, unknown>) {
  await prisma.emailSettings.upsert({
    ...
    update: {
      ...
      smtpUser: "",
      smtpPassword: "",
      fromEmail: "no-reply@suppo-test.io",
      fromName: "Suppo Helpdesk E2E",
      sesAccessKey: null,
      sesSecretKey: null,
...
test("고객이 티켓을 생성하면 고객/내부 이메일이 outbox에 적재된다", async ({ page }, testInfo) => {
  ...
  await test.step("티켓 작성 및 제출", async () => {
    ...
    await page.getByLabel("이메일").fill("email-e2e@suppo-test.io");
