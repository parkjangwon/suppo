...
test("관리자가 티켓 상세에서 답변을 작성하고 상태를 변경한다", async ({ page }, testInfo) => {
  ...
  await test.step("내부 메모 작성 및 전송", async () => {
    // Wait for loading to finish (button text returns to "전송", not "전송중...")
    await expect(page.getByRole("button", { name: "전송", exact: true })).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.getByRole("button", { name: "전송", exact: true })).toBeEnabled({ timeout: 5000 });
    await page.getByRole("button", { name: "전송", exact: true }).click();
    await captureStep(page, testInfo, "내부 메모 작성 및 전송");
  });

  await test.step("상태 변경 (OPEN → IN_PROGRESS)", async () => {
    // Radix UI Select - trigger shows current status text "열림" (OPEN)
    const patchDone = page.waitForResponse(
      (resp) => resp.url().includes("/api/tickets/") && resp.request().method() === "PATCH",
      { timeout: 5000 }
    );
    await page.getByRole("combobox").filter({ hasText: "열림" }).click();
