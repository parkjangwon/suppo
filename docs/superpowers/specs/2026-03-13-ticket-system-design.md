...
Suppo는 SaaS 제품에 대한 기술지원 및 VOC(Voice of Customer) 처리를 위한 티켓 시스템이 필요하다. 고객이 웹 페이지에서 문의를 작성하면, 상담원(5~15명)에게 자동 할당되어 처리되는 구조이다. 단일 제품 대상이며, 별도의 멀티테넌트 구조는 불필요하다.
