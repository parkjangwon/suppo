# Crinity Helpdesk Design System Spec

## Context

이 문서는 Crinity Helpdesk의 현재 UI를 감사한 뒤, 앞으로 제품 전반에 적용할 단일 디자인 시스템 규칙을 정의한다. 목표는 다음 네 가지다.
...
Crinity Helpdesk는 "실무형 운영 도구 + 친절한 고객 접점"이라는 이중 성격을 가진다. 따라서 기본 톤은 차분하고 데이터 지향적이어야 하고, 퍼블릭 화면에서도 과도하게 마케팅스러워지면 안 된다.
