...
export function validateIssueUrl(url: string, provider?: GitProvider): string {
  ...
  switch (provider) {
    ...
    case "CODECOMMIT":
      ...
      if (
        !/^https:\/\/[^.]+\.console\.aws\.amazon\.com\/codesuite\/codecommit\/repositories\//.test(
          trimmed,
        )
      ) {
...
function calculateTemplateScore(
  ...
  // 4. 우선순위 기반 (+10)
  if (ticket.priority === "HIGH" && template.title.includes("긴급"))
    score += 10;
...
export async function calculateTemplateEffectiveness(templateId: string) {
  ...
  return {
    ...
    avgCustomerRating:
      usages.reduce((a, b) => a + (b.customerRating || 0), 0) /
...
현재 Suppo Helpdesk의 Git 연동과 템플릿 시스템은 기본적인 기능은 구현되어 있으나, 운영성과 UX 측면에서 개선의 여지가 있습니다.
